...
function getMostCommonGenres(books) {
  ...
  let topGenre = common.sort((countA, countB) => countA.count < countB.count ? 1 : -1)
...
function getMostPopularBooks(books) {
  ...
  for (let book of books) {
    const title = popular.find((currentBook) => currentBook.name === book.title);  
   
    popular.push({ name: book.title, count: book.borrows.length });
    
    };

  let mostPopular = popular.sort((countA, countB) => countA.count < countB.count ? 1 : -1);
